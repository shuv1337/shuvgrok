...
impl TokenRefresher for CodexRefresher {
    async fn refresh(&self, _reason: RefreshReason) -> RefreshOutcome {
        ...
        if !status.is_success() {
            ...
            tracing::warn!(status = %status, body = %body, "OpenAI token refresh failed");
        ...
        let resp: OpenAiRefreshResponse = match response.json().await {
            Ok(r) => r,
            Err(e) => {
                return RefreshOutcome::TransientFailure {
        ...
        let account_id = extract_chatgpt_account_id(&resp.access_token)
            .or_else(|| current_auth.account_id.clone());
        ...
        let updated_auth = GrokAuth {
            ...
            create_time: now,
            user_id: account_id.clone().unwrap_or(current_auth.user_id),
            account_id,
