...
fn apply_subagent_fanout(
    ...
    for (position, id) in fanout.pool.iter().enumerate() {
        let Some(entry) = crate::agent::config::find_model_by_id(&ctx.available_models, id) else {
    ...
    let start_idx = FANOUT_ROTATION.fetch_add(1, std::sync::atomic::Ordering::Relaxed) % pool_len;
