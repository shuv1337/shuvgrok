...
use super::auth::{
    dispatch_cancel_login, dispatch_login, dispatch_login_with_provider, dispatch_logout,
...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::LoginWithProvider(provider) => dispatch_login_with_provider(app, provider),
