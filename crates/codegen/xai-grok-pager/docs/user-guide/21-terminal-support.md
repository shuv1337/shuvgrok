...
ShuvGrok runs as a full-screen TUI. It relies on terminal support for color,
...
Grok cannot start, run `shuvgrok doctor` in your shell. Use `shuvgrok doctor --json`
...
`shuvgrok doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `shuvgrok doctor fix dcs-passthrough --yes`.
...
through `shuvgrok wrap`, for example `shuvgrok wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `shuvgrok wrap`, Grok shows the one-time tip
...
For repeated SSH use, Doctor offers `shuvgrok doctor fix ssh-wrap`. It also shows
...
> **Warning**: `shuvgrok wrap` is experimental and may not work in every setup.
...
`shuvgrok --no-alt-screen` to confirm inline mode works.
...
Run `shuvgrok doctor`, or run `/doctor` while voice mode is on. The **Voice** section
