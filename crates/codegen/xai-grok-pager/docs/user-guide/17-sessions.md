...
```bash
shuvgrok --resume <session-id-or-title>
```
...
Run `shuvgrok --resume` without a value to resume the most recent session for the current directory.

### From the Welcome Screen

When you launch `shuvgrok`, the welcome screen lists recent sessions for the current directory. Select one to resume it.
...
- Shell version
- Auth method (OAuth vs API key; API-key sessions also suggest `shuvgrok login` for SuperGrok)
- Session ID
- Working directory
...
On the Session info tab, click a value to copy it, or drag to select a range (same highlight as the tool viewer). `c` copies the session ID and `y` copies the whole block. Copy uses the same clipboard route as the rest of Grok, including `shuvgrok wrap`.
...
shuvgrok -p "Continue where we left off" -r <session-id-or-title>

# Continue the most recent session in the current directory
shuvgrok -p "What were we doing?" -c
```
...
```bash
shuvgrok -p "Hello" --output-format json | jq -r '.sessionId'
```
...
## The shuvgrok sessions Subcommand

List or search sessions from the command line. `shuvgrok sessions` requires a subcommand:

```bash
# List recent sessions for the current directory
shuvgrok sessions list

# Limit the number of results (default 20)
shuvgrok sessions list --limit 50

# Search sessions by keyword (matches titles and prompts)
shuvgrok sessions search "rate limit"
```

`shuvgrok sessions list` shows sessions for the current working directory, grouped by worktree label. Each row lists the session ID, the creation and update dates, the source status, and the summary. `shuvgrok sessions search` combines a local SQLite index with remote results.
...
Resume a session in a fresh worktree with `shuvgrok -w -r <session-id>`.

### Checking Disk Usage

`shuvgrok du` (alias: `shuvgrok disk-usage`) reports what the shuvgrok home (`~/.grok`) uses on disk. It lists each top-level directory, largest first, then each worktree with its size, type, age, label, and path. Worktrees the registry does not track appear as `untracked`. Pass `--json` for the same report as machine-readable output.
...
To reclaim space, run `shuvgrok worktree gc --max-age 7d --dry-run`, then the same command without `--dry-run`. Without `--max-age`, gc expires nothing, and it keeps a worktree whose work it cannot find elsewhere, naming each one.
Untracked rows are not in the registry, so gc never visits them. Remove one with `shuvgrok worktree rm --dry-run <path>`, then without `--dry-run`.
```

`AGE` is the value `shuvgrok worktree gc` measures: time since the worktree was last accessed, or since it was created when that is more recent. Session and agent activity update it; a shell or editor left open in the directory does not. An untracked worktree has no registry entry, so its age comes from the newest file underneath it.

Sizes are physical block counts on Unix and logical file sizes elsewhere, matching what `shuvgrok worktree show` reports. A worktree clone shares storage with its source and each copy counts in full, so the total can exceed both `du -sh` and the space actually in use. When the total exceeds the used space on the volume, the report says so. `--json` carries the same figures as `volume_capacity_bytes` and `volume_available_bytes`.

The report measures a single filesystem, the one holding the shuvgrok home. A directory on any other filesystem stays out of the total and is counted in `other_filesystem_dirs`, and its worktree rows show `-` for size (`null` in `--json`). A top-level symlink to a directory, such as a relocated `worktrees`, is counted in `unfollowed_dir_symlinks`; its target stays out of the total, though the rows below it are still sized. Directories and entries the report could not read are counted in `unreadable_dirs` and `unstatable_entries`. Run `RUST_LOG=debug shuvgrok du` to name each one.
...
The smaller state files -- `summary.json`, `plan.json`, and `signals.json` -- are plain JSON rather than JSONL. JSONL is the source of truth for session content; `shuvgrok sessions search` additionally maintains a local SQLite FTS5 index over session titles and prompts for fast keyword search.
