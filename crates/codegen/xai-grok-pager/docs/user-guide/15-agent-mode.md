...
Agent mode runs Grok as a long-lived server that clients talk to over [ACP](https://agentclientprotocol.com) (JSON-RPC). Use it from IDEs, SDKs, eval harnesses, and custom apps. For a one-shot prompt that prints and exits, use `shuvgrok -p` instead ([headless mode](14-headless-mode.md)).
...
shuvgrok agent --always-approve stdio

# WebSocket server
shuvgrok agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
shuvgrok agent --always-approve --model grok-4.6 stdio
shuvgrok agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
This is a server you run yourself — Grok's hosted cloud sandboxes do not run `shuvgrok agent serve`.
...
```bash
shuvgrok agent --always-approve headless --grok-ws-url wss://your-relay.example.com/ws
```
...
class GrokACPChat {
  ...
  async init() {
    this.proc = spawn("shuvgrok", ["agent", "--always-approve", "stdio"]);
