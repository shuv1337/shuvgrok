...
ShuvGrok discovers file-based personas from these locations, in priority order:
...
> **Note:** ShuvGrok applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, ShuvGrok resolves the effective model and reasoning effort in this order, highest priority first:
...
ShuvGrok manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
ShuvGrok also discovers roles from `.grok/roles/*.toml` and personas from `.grok/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

ShuvGrok shows running and finished work in side panes on the agent screen:
