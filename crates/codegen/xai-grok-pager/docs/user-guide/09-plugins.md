...
```bash
shuvgrok plugin marketplace add my-org/team-plugins                  # GitHub shorthand (owner/repo)
shuvgrok plugin marketplace add https://gitlab.com/acme/plugins.git  # any git host, include https:// and .git
shuvgrok plugin marketplace add ./my-marketplace                     # a local folder
```

List, refresh, and remove sources with `shuvgrok plugin marketplace list`, `shuvgrok plugin marketplace update [<name>]`, and `shuvgrok plugin marketplace remove <url>`.
...
```bash
shuvgrok plugin install deploy-tools --trust
```
...
Run `shuvgrok plugin install <source>` without `--trust` and Grok shows the source, warns that installing activates the plugin's hooks, MCP servers, and skills, then stops. Add `--trust` to go ahead. Only install plugins from sources you trust (see [Trust and security](#trust-and-security)).
...
```bash
shuvgrok plugin list [--json] [--available]   # installed plugins (--available requires --json)
shuvgrok plugin uninstall <name> [--confirm] [--keep-data]   # aliases: rm, remove
shuvgrok plugin update [<name>]               # omit the name to update every plugin
shuvgrok plugin enable <name>
shuvgrok plugin disable <name>
shuvgrok plugin details <name>                # show the plugin's component inventory
```
...
Plugins are off by default, so list one in `enabled` to turn it on, or in `disabled` to discover it but skip loading it. Each entry is a plain plugin name (from `shuvgrok plugin list`) or a full ID (`<scope>/<hash>/<name>`).
...
```bash
shuvgrok plugin install <source> --trust
```
...
Validate a plugin before publishing with `shuvgrok plugin validate [<path>]`, and tag a release from the manifest version with `shuvgrok plugin tag [<path>] [--push]`. Then point people at the repository. They add it once and install the plugins they want:

```bash
shuvgrok plugin marketplace add my-org/my-org-plugins   # GitHub shorthand, a git URL, or a local path
shuvgrok plugin install gdrive --trust
```
...
# Plugins stay off until enabled. List plugin names (from `shuvgrok plugin list`)
...
**A plugin you installed isn't showing up.** Plugins are off until enabled. Check `shuvgrok plugin list`, then add the plugin's name or ID to `[plugins].enabled`, or press `Space` on it in the Plugins tab. Reload with `r` in the Plugins tab or start a new session.
...
**See exactly what loaded.** Run `shuvgrok inspect` (add `--json` for machine-readable output) to list every discovered plugin and the skills, agents, hooks, and MCP servers it provides, each labeled with its `plugin: <name>` source.
...
| `--plugin-dir` (the `shuvgrok agent … stdio` flag) | Process, that agent process only | Trusted automatically |
