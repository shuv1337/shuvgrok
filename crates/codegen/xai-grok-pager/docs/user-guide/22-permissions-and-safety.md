...
```bash
shuvgrok -p "Run the tests" --always-approve
...
```bash
shuvgrok --always-approve -p "Run the test suite"
shuvgrok --permission-mode auto
shuvgrok agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
shuvgrok -p "Deploy the service" --always-approve --deny 'Bash(rm -rf *)'
```
...
```bash
shuvgrok -p "Review the API changes" \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)' \
  --allow 'Read' \
  --allow 'Grep' \
...
```bash
shuvgrok -p "Implement the feature using only git and GitHub CLI" \
  --allow 'Read' \
  --allow 'Grep' \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)'
