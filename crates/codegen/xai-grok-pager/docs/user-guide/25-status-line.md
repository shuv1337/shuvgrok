...
- **The timer belongs to the mode that runs a script.** `refresh_interval` under `builtin` schedules nothing and is reported through `shuvgrok inspect`; under `disabled` it is off with everything else.
...
- **A message in the row.** A row beginning `[ui.status_line]` means Grok could not use that section as written: it either names the key it could not read, or names what the mode you chose still needs. `shuvgrok inspect` lists the same problems, including keys this version does not know, which is where to look when the row is switched off. Everything it could read still applies, and Grok leaves the section as you wrote it rather than rewriting one it cannot read. Setting `type = "disabled"` removes the row and the message.
- **A blank row that never fills.** The agent is not sending status updates, which usually means a `shuvgrok` or leader process older than this client. Restart the leader or update Grok.
