...
shuvgrok -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
shuvgrok -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
shuvgrok -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
shuvgrok -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
shuvgrok -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
shuvgrok -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny sudo
shuvgrok -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Allow all bash commands (auto-approve without prompting)
shuvgrok -p "Build the project" --allow "Bash"
```
...
The emitted error subtypes are `error_max_turns`, `error_during_execution`, and `error_max_structured_output_retries`. The schema's `error_max_budget_usd` subtype is never emitted, because shuvgrok has no budget feature.
...
`modelUsage` carries the per-model token and cost fields shuvgrok tracks, plus `webSearchRequests` attributed to the active model. The reducer tracks a single global web-search count rather than per-model, so the whole count lands on the current or last model and other rows stay `0`. A per-model `modelUsage.*.costUSD` is `0` when that model's cost is unknown or withheld. This is the same fail-closed-to-zero behavior as the top-level `total_cost_usd`. The `json` format omits cost floats entirely when partial, but this stream keeps the field present and `0`. `contextWindow` is the current model's real total context window (the same value shuvgrok uses for auto-compaction), and it appears only on the current model's row. Other rows omit it, and so does the current row when the window is unknown. `maxOutputTokens` has no shuvgrok catalog, so that key is omitted entirely. `modelUsage` is `{}` when no per-model breakdown is available.

Like `streaming-json`, this stream is read only. Tool approvals and other bidirectional flows use the ACP interface (`shuvgrok agent`).

---

## Session Management in Headless Mode

By default, each `shuvgrok -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
shuvgrok -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
shuvgrok -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
shuvgrok -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
shuvgrok -p "Remember: the secret number is 42" --output-format json
# Output includes "sessionId": "abc123"

# Resume that exact session
shuvgrok -p "What's the secret number?" --resume abc123
```
...
```bash
shuvgrok -p "Continue where we left off" -c
```
...
shuvgrok -p "Generate a README" > README.md

# Parse JSON output with jq
shuvgrok -p "List files" --output-format json | jq -r '.text'
```
...
shuvgrok -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
shuvgrok --prompt-file ./prompt.txt
```
...
```bash
shuvgrok -p "Review changes for bugs and security issues." \
...
```bash
shuvgrok -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
for file in src/*.js; do
  shuvgrok -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
class GrokChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["shuvgrok", "-p", prompt, "-m", model, "--cwd", self.cwd,
...
RESULT=$(shuvgrok -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
```bash
shuvgrok -p "Format all files" --always-approve
shuvgrok -p "Run the tests and fix any failures" --cwd ~/projects/my-app --always-approve
```
...
shuvgrok -p "Run the test suite" --yolo
```
...
- Combine `--yolo` with `--rules` to set guardrails: `shuvgrok -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug shuvgrok -p "..." 2> debug.log`.
...
- Resume: `shuvgrok -p "continue" --resume "<id>"` or `shuvgrok -p "continue" --continue`
