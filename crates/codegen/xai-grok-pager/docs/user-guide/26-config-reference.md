...
Run `shuvgrok inspect` or `shuvgrok inspect --json` to see which files and values won.
...
| `ui.contextual_hints.ssh_wrap` | `boolean` | `yes` | `user` | Recommend `shuvgrok wrap` when SSH lacks a clipboard sink. |
...
| `ui.screen_mode` | `fullscreen / minimal` | `yes` | `user` | Default render mode for plain `shuvgrok`. Restart required. |
...
| --- | --- |
| A developer sets a key you pinned | The pinned value applies. `shuvgrok inspect` lists the requirements file that contributed. |
...
Run `shuvgrok inspect` on the developer's machine. It lists every config file that contributed, including requirements and managed layers, so a policy that is not applying is visible in one command.
