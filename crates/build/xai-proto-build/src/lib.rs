...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        // protoc writes the dependency listing to a real file. `/dev/stdout`
        // and `/dev/null` would be terser, but they do not exist on Windows,
        // where protoc fails with "No such file or directory" and the build
        // script panics before any codegen happens.
        let scratch = std::env::var_os("OUT_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(std::env::temp_dir);
        let dependency_out = scratch.join("xai-proto-build.d");
        let descriptor_out = scratch.join("xai-proto-build.descriptor");
        ...
        for proto in protos {
            ...
            // The listing is Makefile-shaped: `<target>: <dep> <dep> \`. The
            // target is the descriptor path we just passed in. Matching that
            // exact string rather than splitting on ':' matters on Windows,
            // where a drive letter puts a colon inside the path itself.
            let prefix = format!(
                "{}:",
                descriptor_out
                    .to_str()
            ...
            for line in iter::once(rem).chain(lines) {
                ...
                // protoc emits native separators, so normalize before
                // matching this well-known-types path.
                if line
                    .replace('\\', "/")
